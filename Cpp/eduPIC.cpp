...
void do_one_cycle (void){
    ...
    for (t=0; t<N_T; t++){         // a RF period is divided into N_T equal time intervals (time step DT_E)
        ...
        e_density.front() *= 2.0;                               // point at two edges have 1/2 control volume
        e_density.back()  *= 2.0;
        ...
        for(k=0; k<N_e; k++){                      // move all electrons in every time step
            ...
            if (measurement_mode) {
                ...
                counter_e_xt[p*N_XT+t_index]   += (1.0-rmod);    // <=> counter_e_xt[p][t_index]
                counter_e_xt[(p+1)*N_XT+t_index] += rmod;        // <=> counter_e_xt[p+1][t_index]
                ue_xt[p*N_XT+t_index]   += (1.0-rmod) * mean_v;
                ue_xt[(p+1)*N_XT+t_index] += rmod * mean_v;
                v_sqr  = mean_v * mean_v + vy_e[k] * vy_e[k] + vz_e[k] * vz_e[k];    // vy, vz ???
                ...
                if ((MIN_X < x_e[k]) && (x_e[k] < MAX_X)){                // Define center belong to [1.125,1.375]*E-2, mid is 1.25e-2=L/2
